...
const volumeRoute = new Hono<{ Bindings: { DATABASE_URL: string; JWT_TOKEN: string }; Variables: { userId: any } }>();
...
volumeRoute.get("/:weekId", authMiddleware, async (c)=>{
    ...
    if (!validUser) {
        return c.json({
            message: "Week not found or you don't have permission to read volume for this week",
            volume: [],
        }, 404)
    }
